...
{
  # keep_idx <- usvi_prok_filled.taxa.df %>%
    # dplyr::filter(grepl(paste0(keep, collapse = "|"), Genus)) %>%
    # droplevels %>%
    # dplyr::select(asv_id) %>%
    # unlist
  # usvi_selected_genus.mat <- usvi_prok_asvs.df %>%
  ...
  #   dplyr::filter(asv_id %in% keep_idx) %>%
  #   tidyr::pivot_longer(., cols = !c(asv_id),
  #                       names_to = "sample_ID",
  #                       values_to = "counts") %>%
  #   dplyr::inner_join(., (usvi_selected_metadata %>%
  #                           tibble::rownames_to_column(var = "metab_deriv_label") %>%
  #                           dplyr::select(metab_deriv_label, sample_id) %>%
  #                           droplevels),
  #                     by = c("sample_ID" = "sample_id"), relationship = "many-to-many", multiple = "all") %>%
  #   dplyr::select(-sample_ID) %>%
  #   dplyr::distinct(asv_id, metab_deriv_label, .keep_all = TRUE) %>%
